...
public class PNM implements IImageFile {
	...
	public Image read(String fileName, Display display) throws Exception {
		...
		if (m_ascii) {
			...
			switch(m_imageType) {
			case Picsi.IMAGE_TYPE_BINARY: return readPBM(in, display);
			case Picsi.IMAGE_TYPE_GRAY: return readPGM(in, display);
			case Picsi.IMAGE_TYPE_RGB: return readPPM(in, display);
			default:
				in.close();
			...
			switch(m_imageType) {
			case Picsi.IMAGE_TYPE_BINARY: return readBinPBM(raf, display);
			case Picsi.IMAGE_TYPE_GRAY: return readBinPGM(raf, display);
			case Picsi.IMAGE_TYPE_RGB: return readBinPPM(raf, display);
			default:
				raf.close();
				throw new Exception("Read PNM: Wrong image type");
			}
