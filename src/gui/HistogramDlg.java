...
public class HistogramDlg extends Dialog {
    ...
    public Object open(TwinView views) {
        ...
        m_canvas.addPaintListener(new PaintListener() {
			...
			public void paintControl(PaintEvent event) {
				...
				final int max = (logBtn.getSelection()) ? (int)Math.round(Math.log(m_stat.getMax())) : m_stat.getMax();
				final RGB[] colors = m_image.getImageData().palette.colors;
				...
				case Picsi.IMAGE_TYPE_RGB:
					...
					for(int v: m_hist) {
						...
						gc.fillRectangle(x, h - height, w, height);
						// draw color
						gc.setBackground(new Color(m_image.getDevice(), (m_selectedChannel == 0) ? pixel : 0, (m_selectedChannel == 1) ? pixel : 0, (m_selectedChannel == 2) ? pixel : 0));
						gc.fillRectangle(x, h, w, colorHeight);
						x += w;
						pixel++;
					}
    ...
    public void update(boolean hasOutput, TwinView views) {
    	...
    	if (m_outputBtn.getSelection()) {
    		m_image = views.getSecondImage();
    		m_imageType = views.getSecondImageType();
    	} else {
	    	m_image = views.getFirstImage();
    	...
    	ImageData inData = m_image.getImageData();
    	if (m_imageType == Picsi.IMAGE_TYPE_RGB) {
    		m_hist = ImageProcessing.histogramRGB(inData, m_selectedChannel);    		
    	} else {
    		m_hist = ImageProcessing.histogram(inData, 1 << Math.min(8, inData.depth));
    	}
