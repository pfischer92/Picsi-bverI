...
public class MainWindow {
	...
	private void createWindowMenu(Menu menuBar) {
		...
		final int SHOWHISTOGRAM = 6;
		final int SHOWPSNR = 7;
		final int SHOWWAVES = 8;
		...
		item.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent event) {
				if (!m_views.isEmpty()) {
					Image image1 = m_views.getFirstImage();
					Image image2 = m_views.getSecondImage();
					int imageType = m_views.getFirstImageType(); assert imageType == m_views.getSecondImageType();
					double[] psnr = ImageProcessing.psnr(image1.getImageData(), image2.getImageData(), imageType);
	...
	private void setTitle(String filename, int fileType) {
		m_shell.setText(Picsi.createMsg(Picsi.APP_NAME + " - {0} ({1})", new Object[]{filename, ImageFiles.fileTypeString(fileType)}));		
	}
